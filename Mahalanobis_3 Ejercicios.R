...
#-------------------------------------------------------------
# El método de distancia Mahalanobis mejora el método clásico de 
# distancia de Gauss eliminando el efecto que pueden producir la 
# correlación entre las variables a analizar.
...
#3. Dar interpretación.


install.packages("datos")
library(datos)

x<- data.frame(datos::fiel)
dim(x)
str(x)
summary(x)
...
# Ordenar los datos de mayor a menor distancia, según la métrica de Mahalanobis.
mah.ordenacion <- order(mahalanobis(x, colMeans(x), cov(x)), decreasing=TRUE)
mah.ordenacion

# Generar un vector boleano los dos valores más alejados segun la distancia Mahalanobis.
outlier2 <- rep(FALSE , nrow(x))
outlier2[mah.ordenacion[1:num.outliers]] <- TRUE
